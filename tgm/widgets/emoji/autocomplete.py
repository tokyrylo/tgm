...
class EmojiAutocomplete(Horizontal):
    ...
    def is_showing(self) -> bool:
        return self.has_class("visible") and bool(self._results)
    ...
    def show_results(self, query: str) -> None:
        ...
        if not term:
            self.hide()
            return
        self.run_worker(self._search_and_render(term), exclusive=True, group="emoji-ac")

    async def _search_and_render(self, term: str) -> None:
        results = search_emojis(term, 12)
        if not results:
            self.hide()
            return
        ...
        self._selected = (
            next((i for i, (_, name, _) in enumerate(results) if name == prev_name), 0)
            if prev_name
            else 0
        )
